...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/sdm-hotspot-effort-correction`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/sdm-hotspot-effort-correction:latest` over HTTPS. |
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/sdm-hotspot-effort-correction:latest`.
